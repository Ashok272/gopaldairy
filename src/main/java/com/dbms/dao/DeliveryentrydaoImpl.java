...
public class DeliveryentrydaoImpl implements Deliveryentrydao{
	...
	public void saveDentry(Deliveryentry deliveryentry, Integer invid) {
		String sql="insert into Deliveryentry(invid,address,city,state,pincode,phone_no,isdelivered) values(?,?,?,?,?,?,?)";
		jdbcTemplate.update(sql, new Object[] {invid,deliveryentry.getAddress(),deliveryentry.getCity(),deliveryentry.getState(),deliveryentry.getPincode(),deliveryentry.getPhone_no(),"No"});
	}
	@Transactional
